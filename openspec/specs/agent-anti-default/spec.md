# agent-anti-default Specification

## Purpose

Defines how coding agents (and humans using the same rules) MUST behave when implementing UI, Desktop, and other presentation layers: no invented product defaults, explicit contracts only, thin adapters over `core::Session`.
## Requirements
