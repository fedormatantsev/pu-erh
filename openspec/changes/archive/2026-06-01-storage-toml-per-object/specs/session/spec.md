...
- **WHEN** a session is opened with a storage directory that includes block version records
...
- **WHEN** a session is opened for a new knowledge base (missing storage directory) and the first save occurs
...
- **AND** saving exports version records from the knowledge base to the storage directory
...
- **WHEN** a session is opened from an existing storage directory, only read queries are executed, and no mutations occur
- **THEN** the storage directory on disk is unchanged
